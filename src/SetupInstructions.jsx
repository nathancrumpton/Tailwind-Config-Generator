...
const StepItem = ({ number, title, description, children, isLast }) => (
  <div className="pb-28 relative">
    <div className="flex items-start">
      <div className="flex-shrink-0 flex items-center justify-center w-5 h-5 rounded-md bg-blue-600 text-white font-bold mr-4 z-10 text-xs">
        {number}
      </div>
      <div className="flex-grow">
        <h3 className="text-base font-semibold mb-1 text-blue-400 text-left">{title}</h3>
        {description && (
    ...
    {!isLast && <div className="absolute left-2.5 top-5 bottom-0 w-px bg-gray-800 -z-10"></div>}
  </div>
);
...
const SetupInstructions = () => {
  ...
  return (
    <>
      {/* Navigation */}
      <nav className="bg-gray-950/80 backdrop-blur-sm flex justify-between fixed top-0 w-full z-50 p-4 border-b border-gray-800">
        <div className="flex items-center gap-2">
          <TailwindCss variant="wordmark" className="h-4 w-auto" />
          <span className="bg-blue-900/40 text-blue-300 text-[10px] px-2 py-0.5 rounded-full border border-blue-600/30">
            v4.3.3
          </span>
        </div>
      </nav>

      <div className="p-8 pt-32 max-w-4xl mx-auto relative overflow-hidden">
        <div
          aria-hidden="true"
          className="pointer-events-none absolute left-1/2 top-0 -z-10 h-[420px] w-[420px] -translate-x-1/2 rounded-full bg-blue-600/20 blur-[110px]"
        />
        <div>
          <h1 className="text-6xl font-bold tracking-tighter text-balance mt-14 mb-8 text-center text-white">
